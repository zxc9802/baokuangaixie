...
6. 标题简洁有力，脚本适合口播。script 必须按内容推进分段，段落之间使用换行符；超过 500 字时至少分为 4 段，禁止整篇挤在同一段。
7. 每条脚本应包含吸引注意的开场、具体使用场景、产品卖点展开和自然收束，并遵守上方目标时长与字数。
...
4. script 必须按内容推进分段，段落之间使用换行符；超过 500 字时至少分为 4 段，禁止整篇挤在同一段。
